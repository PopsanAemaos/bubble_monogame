...
using bubble_mono.GameObjectsNormal;
using Microsoft.Xna.Framework.Media;

namespace bubble_mono.Screen
{
    class PlayScreenNormal : _GameScreen
    {
        ...
        private BubbleNormal[,] bubble = new BubbleNormal[9, 8];
        private Color _Color;
        private Random random = new Random();
        private GunNormal gunNormal;
        ...
        public void Initial()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                {
                    Console.WriteLine("dscsdcsdcsdcsdc");                
                    int index = random.Next(0, 4);
                    bubble[i, j] = new BubbleNormal(BubleList[index], BubleList)
                    {
                        Name = "Bubble",
                        Index = index,
            ...
            gunNormal = new GunNormal(GunTexture, BubleList)
            {
                Name = "GunNormal",
        ...
        public bool CheckWin(BubbleNormal[,] bubble)
        {
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 8 - (i % 2); j++)
        ...
        public override void Update(GameTime gameTime)
        {
            if (!gameOver && !gameWin)
            {
                ...
                gunNormal.Update(gameTime, bubble);
