...
{
	public class BubbleHard : _GameObjectsHard
    {
		public float Speed;
		public float Angle;
		...
		public BubbleHard(Texture2D texture, List<Texture2D> bubble) : base(texture) {
		...
		private void DetectCollision(BubbleHard[,] gameObjects) {
				for (int i = 0; i < 9; i++) {
					for (int j = 0; j < 8; j++) {
						...
						if (gameObjects[i, j] != null && !gameObjects[i, j].IsActive)
                        {
                            ...
                            {                          
								...
								if (Singleton.Instance.removeBubble.Count >= 3 || Singleton.Instance.IsSpecialBallVector) {
									...
									{
										gameObjects[(int)v.Y, (int)v.X] = new BubbleHard(bubbleTextureList[Index], bubbleTextureList)
										{
											Name = "Bubble",
											Index = Index,
		...
		public int calDistance(BubbleHard other) {
		...
		public void CheckRemoveBubble(BubbleHard[,] gapositionObjects, int IndexTarget, Vector2 position)
		{
        ...
        public void CheckSpecialBubble(BubbleHard[,] gapositionObjects, int IndexTarget, Vector2 position)
        {
            if (gapositionObjects[(int)position.Y, (int)position.X].Index == 7)
            {
                ...
                if (gapositionObjects[(int)position.Y, (int)position.X].Index == 6)
                {
