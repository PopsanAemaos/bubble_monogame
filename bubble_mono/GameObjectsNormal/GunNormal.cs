...
{
	class GunNormal : _GameObjectsNormal
	{
		private Random random = new Random();
		private BubbleNormal BubbleOnGun;
		private int _Index;
		private float angle;
		private List<Texture2D> bubbleTextureList;

		public SoundEffectInstance _deadSFX, _stickSFX, _thunderSFX, _boomSFX;
		public GunNormal(Texture2D texture, List<Texture2D> bubble) : base(texture) {
			bubbleTextureList = bubble;
            _Index = random.Next(0, 6);
        }
		...
		public override void Update(GameTime gameTime, BubbleNormal[,] gameObjects) {
			...
			if (Singleton.Instance.MouseCurrent.Y < Singleton.Instance.Diemensions.Y - bubbleTextureList[_Index].Height) {
				...
				if (!Singleton.Instance.Shooting && Singleton.Instance.MouseCurrent.LeftButton == ButtonState.Pressed && Singleton.Instance.MousePrevious.LeftButton == ButtonState.Released) {
                    BubbleOnGun = new BubbleNormal(bubbleTextureList[_Index], bubbleTextureList) {
						Name = "Bubble",
